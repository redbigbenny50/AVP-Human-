...
/**
 * Strategy for using drinkable potions of regeneration.
 */
public class DrinkableRegenPotionStrategy implements HealingStrategy {

    @Override
    public boolean isValidItemStack(ItemStack itemStack) {
        return itemStack.is(Items.POTION) && HealingStrategyUtil.hasMobEffect(itemStack, MobEffects.REGENERATION);
    }
    ...
    public double score(LivingEntity livingEntity, ReadableWorldState worldState, ItemStack itemStack) {
        var healthRatio = worldState.getOrDefault(GOAPSensors.HEALTH_RATIO.key(), 1.0F);
        var healing = HealingStrategyUtil.estimateRegenHealing(itemStack);
        ...
        // Regen takes time to heal, so slightly penalize it for urgent situations.
